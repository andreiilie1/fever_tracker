...
def fetch_medication_names() -> pd.DataFrame:
    with _get_connection() as conn:
        df = pd.read_sql_query("SELECT id, name FROM medication_names ORDER BY LOWER(name) ASC", conn)
    return df
...
def add_medication_name(name: str) -> int:
    ...
    with _get_connection() as conn:
        conn.execute("INSERT OR IGNORE INTO medication_names(name) VALUES (?)", (cleaned,))
        row = conn.execute("SELECT id FROM medication_names WHERE name = ?", (cleaned,)).fetchone()
        return int(row[0])
...
def update_medication_name(row_id: int, name: str) -> None:
    ...
    if not cleaned:
        raise ValueError("Medication name cannot be empty.")
    with _get_connection() as conn:
        conn.execute("UPDATE medication_names SET name = ? WHERE id = ?", (cleaned, int(row_id)))
...
# Write operations
def add_measurement(recorded_at_iso: str, temperature_c: float, notes: Optional[str] = None) -> int:
    with _get_connection() as conn:
        cur = conn.execute(
...
def add_medication(given_at_iso: str, med_name: str, dose_desc: Optional[str] = None, notes: Optional[str] = None) -> int:
    with _get_connection() as conn:
        cur = conn.execute(
...
def update_measurement(row_id: int, recorded_at_iso: str, temperature_c: float, notes: Optional[str]) -> None:
    with _get_connection() as conn:
        conn.execute(
...
def update_medication(row_id: int, given_at_iso: str, med_name: str, dose_desc: Optional[str], notes: Optional[str]) -> None:
    with _get_connection() as conn:
        conn.execute(
